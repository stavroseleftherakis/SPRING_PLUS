function vector_num_estimated_paths = MPM_delayEstimator(CSI_all_packets,P,unit_circle_toll)

num_packets = size(CSI_all_packets,1);
antennas = size(CSI_all_packets,3);
...
for num_packet = 1:num_packets
    ...
    H_iscell = iscell(CSI_all_packets);
    if (H_iscell)
        H_tmp = CSI_all_packets{num_packet};
    else
        H_tmp = squeeze(CSI_all_packets(num_packet, :, :, :));
    end
